...
const jsonPathFor = modelName => {
  const fileName = `${common.pluralize(lodash.toLower(modelName))}`;
...
const transformToPredefineSeed = seed => {
  ...
  const predefine$1 = predefine.transformToPredefine(data);
  ...
  lodash.forEach(eweaInternals.PREDEFINE_RELATIONS, (value, key) => {
    ...
    if (hasRelation) {
      ...
      const model = options.ref || eweaInternals.MODEL_NAME_PREDEFINE;
      const array = options.array || false;
      const vals = common.sortedUniq(lodash.split(seed[key], ','));
      const match =
        model === eweaInternals.MODEL_NAME_PREDEFINE
...
const applyTransformsOn = (seed, ...transformers) => {
  ...
  // ensure transformers
  const transforms = common.compact([transformSeedKeys].concat(transformers));
    ...
    then => {
      seedCsv(
        ...
        (error, { finished, feature, next }) => {
          ...
          if (feature && next) {
            // seed feature
            const data = common.mergeObjects(feature, { namespace });
            return predefine.Predefine.seed(data, (err, seeded) => {
...
const seedPermissions = done => {
  ...
  return async.waterfall(
    [
      next => permission.Permission.seed(error => next(error)),
      next => permission.Permission.seed(predefine.listPermissions(), error => next(error)),
    ],
    error => {
...
const seedEventCatalogues = done => {
  logger.debug('Start Seeding Event Catalogues Data');
  return seedPredefine('EventCatalogue', error => {
    logger.debug('Finish Seeding Event Catalogues Data');
    return done(error);
  });
};
...
exports.seedCsv = seedCsv;
exports.seedEventActions = seedEventActions;
exports.seedEventCatalogues = seedEventCatalogues;
